# -*- coding: utf-8 -*-
"""
Created on Thu May  8 15:36:10 2025
...
save_dir = r'E:\Aman\20250810_Piezo_3752_100x_minGlu_KinetiX'
...
#%%Camera properties
core.set_property('KinetiX','CircularBufferAutoSize','OFF')
core.set_property('KinetiX','CircularBufferFrameCount',30)
core.set_property('KinetiX','ExposeOutMode','First Row')
core.set_property('KinetiX','ShutterMode','Pre-Exposure')
#core.set_property('KinetiX','Gain','HIGH')
# core.set_property('KinetiX','OUTPUT TRIGGER PRE HSYNC COUNT','2')
# #core.set_property('KinetiX','MASTER PULSE TRIGGER SOURCE')
cam_props = core.get_device_property_names('KinetiX')
for i in range(cam_props.size()):
    print(cam_props.get(i)+':'+str(core.get_property('KinetiX',cam_props.get(i))))
    
#print(core.get_property('KinetiX','FrameRate'))


str_arr = (core.get_allowed_property_values('KinetiX','Binning'))
...
def set_device_prop(g_l,cy_l,las_l, exp_t,x_p,y_p,z_p,top_left_x,top_left_y,x_size,y_size,fps):
    ...
    #camera
    # core.set_property('KinetiX','SENSOR MODE','AREA')#Global shutter cause we want to caputre high speed motion 
    # core.set_property('KinetiX','OUTPUT TRIGGER KIND[2]','EXPOSURE')
    # core.set_property('KinetiX','OUTPUT TRIGGER POLARITY[2]','POSITIVE')
    # core.set_property('KinetiX','Trigger','NORMAL')
    core.set_roi('KinetiX',top_left_x,top_left_y,x_size,y_size)
    core.set_property('KinetiX','Port','Sub-Electron')
    core.set_exposure('KinetiX',exp_t)
    core.set_property('KinetiX','Binning','1x1')
...
core.stop_sequence_acquisition()
#%% Main
camera_exposure_fluor = 100.0
camera_exposure_BF = 10.0

gfP_light_level = '0.0'
mch_light_level = '27.0'
laser_pow_level = '0.8'
...
set_device_prop(mch_light_level,gfP_light_level , laser_pow_level, camera_exposure_BF, x_pos, y_pos,z_pos, roi_top_lf_x,roi_top_lf_y,roi_x_sz,roi_y_sz,fps)
...
images = piezo_zstack_ts_NIDAQ(num_time_points,step_v,z_pos,z_steps,camera_exposure_fluor)    
en = time.time()
...
save_d = os.path.join(save_d,'wait_time_'+str(wait_time)+'_exposure_time'+str(camera_exposure_fluor)+'_Z_steps_'+str(z_steps)+'_stepV_'+str(step_v)+'_number_of_TPs_'+str(num_time_points))
if(not(os.path.exists(save_d))):
    os.makedirs(save_d)
