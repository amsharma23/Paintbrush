...
class SynchronizedCameraSystem:
    ...
    def save_images(self, images: Dict[str, list[dict]], save_folder: str, filename_prefix: str = None) -> Dict[str, str]:
        ...
        for camera_name in images.keys():
            ...
            # Create save folder if it doesn't exist
            if(camera_name=='Kinetix1'):
                save_path = Path(os.path.join(save_folder,camera_name+'_matrix'))
                save_path.mkdir(parents=True, exist_ok=True)
            elif(camera_name=='Kinetix2'):
                save_path = Path(os.path.join(save_folder,camera_name+'_OMM'))
                save_path.mkdir(parents=True, exist_ok=True)            
    ...
    def continuous_trigger_thread(self,save_folder,full_arr: list = np.arange(0.0,0.63,0.03),rep: int = 1):
        
        full_arr_rep = len(full_arr)*rep
        ...
        logger.info(f"Number of total images per camera should be {(full_arr_rep)}")
        ...
        try:
            with nidaqmx.Task() as trigger_task, nidaqmx.Task() as piezo_task:
                ...
                piezo_task_writer.write_many_sample(full_arr)
                ...
                time.sleep(self.trigger_interval*(full_arr_rep)/1000)
    ...
    def start_synchronized_acquisition(self,save_folder, tps: int = 1, volt_arr: list = np.arange(0.0,0.63,0.03)):
        ...
        try:
            ...
            self.trigger_thread = Thread(target=self.continuous_trigger_thread(save_folder,full_arr,(math.floor(tps/2)-1)), daemon=True)
...
if __name__ == "__main__":
    ...
    save_folder = r'D:\Aman\20250911_SJSC103_EGrich_dualKinetix_TRIF100x_1.5x\run6_zsteps24_tps60'
    save_path = Path(save_folder)
    save_path.mkdir(parents=True, exist_ok=True)
    ...
    laser_int_c = 10
    laser_int_g = 15
    roi_K1 = [[1576,1444],[320,280]]
    roi_K2 = [[1595,1492],[320,280]]
